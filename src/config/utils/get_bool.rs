...
pub(crate) fn get_bool(config: &Config, name: &str, default: bool) -> Result<bool, ConfigError> {
...
mod tests {
	...
	fn read_true() {
		with_git_config(&["[test]", "bool = true"], |git_config| {
			assert_ok_eq!(get_bool(&git_config, "test.bool", false), true);
		});
	}
	...
	fn read_false() {
		with_git_config(&["[test]", "bool = false"], |git_config| {
			assert_ok_eq!(get_bool(&git_config, "test.bool", true), false);
		});
	}
	...
	fn read_default() {
		with_git_config(&[], |git_config| {
			assert_ok_eq!(get_bool(&git_config, "test.bool", true), true);
		});
	}
	...
	fn read_invalid_value() {
		with_git_config(&["[test]", "bool = invalid"], |git_config| {
			assert_err_eq!(
				get_bool(&git_config, "test.bool", true),
